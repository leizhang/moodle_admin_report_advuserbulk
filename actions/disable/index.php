...
//check_action_capabilities('disable', true);

$return = $CFG->wwwroot.'/'.$CFG->admin.'/report/advuserbulk/user_bulk.php';

$langdir = $CFG->dirroot . '/admin/report/advuserbulk/actions/disable/lang/';
$pluginname = 'bulkuseractions_disable';
...
if ($confirm and confirm_sesskey()) {
    ...
    if ($rs = get_recordset_select('user', "id IN ($in)")) {
        while ($user = rs_fetch_next_record($rs)) {
            if ($primaryadmin->id != $user->id and $USER->id != $user->id) {
                ...
                notify(get_string('disablenot', $pluginname, fullname($user, true), $langdir));
            }
        }
        rs_close($rs);
    }
    ...
    notice_yesno(get_string('disablecheckfull', $pluginname, $usernames, $langdir), 'index.php', $return, $optionsyes, NULL, 'post', 'get');
}
